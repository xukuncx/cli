...
| VC | [`references/lark-event-vc.md`](references/lark-event-vc.md) | Catalog of 2 VC EventKeys (`vc.meeting.participant_meeting_ended_v1`, `vc.note.generated_v1`) + field reference + enrichment & degradation semantics (note detail API fills `note_token` / `verbatim_token` / `note_source`; graceful fallback on failure) + source type semantics (meeting) |
