...
name: lark-vc-agent
version: 1.0.0
description: "飞书视频会议 — Agent 参会能力：通过 9 位会议号让当前 user 身份真实加入、离开正在进行的视频会议，并拉取 bot 参会期间的会中事件流（参会人加入/离开、聊天、转写、共享等）。1. 用户提供 9 位会议号要求代为加入/离开会议时使用 +meeting-join / +meeting-leave（写操作，仅 user 身份）。2. 用户需要感知 **正在进行中** 会议的实时事件（谁进来、谁离开、谁发言、谁共享）时使用 +meeting-events。3. Agent 做参会机器人、会中助手、旁听、代为参会等场景时使用本技能。本技能只覆盖会中原子操作；要查已结束会议的纪要 / 参会人快照 / 搜索历史会议请使用 lark-vc 技能。"
metadata:
  requires:
    bins: ["lark-cli"]
