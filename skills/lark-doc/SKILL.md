...
`docs +create` / `docs +update` 并非支持全部 block 类型。引用同步块（`<reference-synced>`）、部分 AddOns 等是 **read-only**，API 返回成功但 block 不会真的写入。撰写 markdown 前请先读：

- [`references/lark-doc-block-type-limits.md`](references/lark-doc-block-type-limits.md) — 已知只读 block 清单、`<!-- Unsupported block type -->` 占位符来源与推荐处理方式。
