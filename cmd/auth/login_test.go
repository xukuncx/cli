...
func TestAuthLoginRun_NoWaitJSONHintIncludesRawURLGuidance(t *testing.T) {
	...
	for _, want := range []string{
		"MUST generate QR code",
		"lark-cli auth qrcode",
		"Prefer PNG QR code (--output)",
		"use ASCII (--ascii) only in pure terminal/CLI environments where images cannot be displayed",
		"This is a required step, do NOT skip it",
		"Display order",
		"place the complete QR code below the URL",
		"opaque string",
		"cannot be modified",
...
func TestAuthLoginRun_JSONDeviceAuthorizationAgentHintIncludesRawURLGuidance(t *testing.T) {
	...
	for _, want := range []string{
		...
		"lark-cli auth qrcode",
		"优先生成 PNG 二维码（--output）",
		"纯终端/CLI 环境无法展示图片时使用 ASCII（--ascii）",
		"这是必须步骤，不要跳过",
		"展示顺序",
		"二维码置于 URL 下方完整展示",
		"URL 输出规则",
